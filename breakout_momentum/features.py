"""
features.py
Technical indicators optimized for breakout momentum trading.
Focuses on volatility expansion, volume surges, and range compression patterns.

Specialized for explosive moves in BNB, SOL, AVAX, and volatile altcoins.
"""
...
def add_features(df: pd.DataFrame, symbol: str = "BTC/USDT",
    ...
    df["donchian_breakout_up"] = (df["close"] >= df["high_20"] * 0.99).astype(int)
    df["donchian_breakout_down"] = (df["close"] <= df["low_20"] * 1.01).astype(int)

    # Breakout strength (how far past the high/low)
    df["breakout_strength_up"] = ((df["close"] - df["high_20"]) / (df["atr_14"] * df["close"] + 1e-8)).clip(-5, 5)
    df["breakout_strength_down"] = ((df["low_20"] - df["close"]) / (df["atr_14"] * df["close"] + 1e-8)).clip(-5, 5)

    # Days/bars since last breakout
    breakout_up_idx = df[df["donchian_breakout_up"] == 1].index
    breakout_down_idx = df[df["donchian_breakout_down"] == 1].index
    df["bars_since_breakout_up"] = 999
    df["bars_since_breakout_down"] = 999

    for i in range(len(df)):
        if i in breakout_up_idx:
            df.loc[df.index[i], "bars_since_breakout_up"] = 0
        elif i > 0:
            df.loc[df.index[i], "bars_since_breakout_up"] = min(df.loc[df.index[i-1], "bars_since_breakout_up"] + 1, 999)

        if i in breakout_down_idx:
            df.loc[df.index[i], "bars_since_breakout_down"] = 0
        elif i > 0:
            df.loc[df.index[i], "bars_since_breakout_down"] = min(df.loc[df.index[i-1], "bars_since_breakout_down"] + 1, 999)

    # BB Width percentile (for compression detection)
    df["bb_width_percentile"] = df["bb_width"].rolling(100).apply(
        lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False
    )

    # Consolidation duration (how long has range been compressed)
    is_compressed = (df["range_compression"] < 0.6).astype(int)
    df["consolidation_duration"] = 0

    streak = 0
    for i in range(len(df)):
        if is_compressed.iloc[i] == 1:
            streak += 1
            df.loc[df.index[i], "consolidation_duration"] = streak
        else:
            streak = 0
            df.loc[df.index[i], "consolidation_duration"] = 0

    # Volume expansion rate (current vs MA)
    df["volume_expansion_rate"] = df["volume"] / (df["volume_sma_20"] + 1e-8)

    # Volatility breakout signal (ATR expanding significantly)
    df["volatility_breakout"] = (df["atr_expansion_rate"] > 1.5).astype(int)
