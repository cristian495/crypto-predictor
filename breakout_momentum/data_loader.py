...
def load_btc_for_context(timeframe: str = "1h",
                         days: int = 730) -> pd.DataFrame:
    """
    Load BTC OHLCV data for market context features (lightweight).
    Only downloads OHLCV, no derivatives data.
    """
    try:
        print(f"  Loading BTC for market context...")
        btc_df = fetch_ohlcv("BTC/USDT", timeframe, days)
        return btc_df
    except Exception as e:
